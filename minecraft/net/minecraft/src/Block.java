...
public class Block
{
    ...
    public boolean shouldSideBeRendered(IBlockAccess par1IBlockAccess, int par2, int par3, int par4, int par5)
    {
		if(GuiIngame.xray) 
	{
		for(int z = 0; z < GuiIngame.idList[GuiIngame.xray_mode].length; z++) {
		int b = GuiIngame.idList[GuiIngame.xray_mode][z];
		if(b == blockID) 
			return true;
		}
  }
    ...
    public int getRenderBlockPass()
    {
     boolean flag = false;
		if(GuiIngame.xray) {
    for(int i = 0; i < GuiIngame.idList[GuiIngame.xray_mode].length; i++) {
    int b = GuiIngame.idList[GuiIngame.xray_mode][i];
    if(b == blockID) 
    flag = true;
    }
  }
  return !GuiIngame.xray || flag ? 0 : 1; 
    }
